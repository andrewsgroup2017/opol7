...
const actions = {
  ...
  findNext(context) {
    let productsQuery = db.products
      .orderBy('title')
      .limit(context.state.pagination.rowsPerPage)
      .startAfter(context.state.pagination.lastProduct)
    findProducts(context, productsQuery)
  },
  ...
  findPrevious(context) {
    let productsQuery = db.products
      .orderBy('title')
      .limit(context.state.pagination.rowsPerPage)
      .endBefore(context.state.pagination.firstProduct)
...
function setupPagination(context, snapshotProducts, snapshotNumberOfProducts) {
  ...
  context.commit(
    SET_PAGINATION_LAST_PRODUCT,
    snapshotProducts.docs[snapshotProducts.docs.length - 1]
  )
  context.commit(
    SET_TOTAL_NUMBER_OF_PRODUCTS,
    snapshotNumberOfProducts.data().count
  )
}
