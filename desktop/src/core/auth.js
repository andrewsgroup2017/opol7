...
const mutations = {
  [types.SET_USER](state, user) {
    if (user) {
      // Send through the bits of the user that we need.
...
const actions = {
    ...
    if (!firebase.auth().currentUser) {
      auth
        ...
        .then(function(result) {
          ...
          dispatch(
            'setNotification',
            getters.defaultNotifications.loggedInMessage
          )
        })
        .catch(function(error) {
          ...
          if (error.code === 'auth/account-exists-with-different-credential') {
            dispatch(
              'setNotification',
              getters.defaultNotifications.accountExistsDifferentWithCredential
            )
  ...
  reAuth({ commit, dispatch, getters }, providerId) {
    ...
    auth.currentUser
      .reauthenticateWithPopup(provider.type)
      .then(function(result) {
        // Disable login button
        commit(types.SET_UI_LOGIN_DISABLED, true)
        // Set logged in within auth state
      ...
      ref.then(function(snapshot) {
        ...
        if (userIdInDatabase) {
          ...
          const userRef = firebase
            ...
            .child(user.uid)
          userRef
            .update({ loggedIn: 'true', uid: user.uid })
            .catch(onReject)
            .then(function() {
